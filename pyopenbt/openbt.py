...
class OPENBT(BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        if type(X) == list:
            X = np.array(X)
            print(
                "Completed list-to-numpy_array conversion for X. Be careful about row/column mixups!"
            )
        if type(y) == list:
            y = np.array(y)
            print(
                "Completed list-to-numpy_array conversion for y. Be careful about row/column mixups!"
            )
    ...
    def _define_params(self):
        ...
        if self.modeltype in [4, 5, 8]:
            ...
            if len(self.uniqy) > 2 or self.uniqy[1] != 0 or self.uniqy[2] != 1:
                sys.exit(
                    "Invalid y.train: Probit requires dichotomous response coded 0/1"
                )
            ...
            self.rgy = [
                -2,
                2,
            ]  # These proprties are ambiguous for these modeltypes by the way...
        ...
        if "xicuts" not in self.__dict__:
            ...
            for feat in range(self.p):
                ...
                self.xi[feat] = [np.arange(1, (self.numcut) + 1) * xinc + minx[feat]]
    ...
    def __write_chunks(self, data, no_chunks, var, *args):
        ...
        for i, ch in enumerate(splitted_data):
            ...
            np.savetxt(
                str(
                    self.fpath / Path(self.__dict__[var + "root"] + str(i + int_added))
                ),
                ch,
                fmt=args[0],
            )
    ...
    def _write_data(self):
        ...
        self.__write_chunks(np.ones((self.n), dtype="int"), splits, "s", "%.0f")
        print(self.fpath)
        if self.X_train.shape[0] == 1:
            ...
            print("2 x variables")
            np.savetxt(
                str(self.fpath / Path(self.chgvroot)),
                [spearmanr(self.X_train, axis=1)[0]],
                fmt="%.7f",
            )
        else:
            print("3+ x variables")
        ...
        for k, v in self.xi.items():
            np.savetxt(str(self.fpath / Path(self.xiroot + str(k + 1))), v, fmt="%.7f")

    def _run_model(self, train=True):
        cmd = "openbtcli" if train else "openbtpred"
        sp = subprocess.run(
            ["mpirun", "-np", str(self.tc), cmd, str(self.fpath)],
    ...
    def predict(self, X, q_lower=0.025, q_upper=0.975, **kwargs):
        ...
        if type(X) == list:
            X = np.array(X)
            print(
                "Completed list-to-numpy_array conversion for the preds. Be careful about row/column mixups!"
            )
        ...
        res = {}
        res["mdraws"] = self.mdraws
        res["sdraws"] = self.sdraws
        res["mmean"] = self.mmean
        res["smean"] = self.smean
        res["msd"] = self.msd
        res["ssd"] = self.ssd
        res["m_5"] = self.m_5
        res["s_5"] = self.s_5
        res["m_lower"] = self.m_lower
        res["s_lower"] = self.s_lower
        res["m_upper"] = self.m_upper
        res["s_upper"] = self.s_upper
        res["q_lower"] = self.q_lower
        res["q_upper"] = self.q_upper
        res["x_test"] = X
    ...
    def _read_in_preds(self):
        ...
        if mdraws[0].ndim == 1:
            self.mdraws = np.stack(mdraws, axis=1)
        else:
        ...
        if sdraws[0].ndim == 1:
            self.sdraws = np.stack(sdraws, axis=1)
        else:
    ...
    def _read_in_vartivity(self, q_lower, q_upper):
        ...
        colnorm = colnorm.reshape(
            self.ndpost, 1
        )  # Will always have 1 column b/c we summed
        ...
        colnormh = colnormh.reshape(
            self.ndpost, 1
        )  # Will always have 1 column since we summed
    ...
    def vartivity(self, q_lower=0.025, q_upper=0.975):
        ...
        cmd = "openbtvartivity"
        sp = subprocess.run(
            [cmd, str(self.fpath)], stdin=subprocess.DEVNULL, capture_output=True
        )
        ...
        res = {}
        res["vdraws"] = self.vdraws
        res["vdrawsh"] = self.vdrawsh
        res["mvdraws"] = self.mvdraws
        res["mvdrawsh"] = self.mvdrawsh
        res["vdraws_sd"] = self.vdraws_sd
        res["vdrawsh_sd"] = self.vdrawsh_sd
        res["vdraws_5"] = self.vdraws_5
        res["vdrawsh_5"] = self.vdrawsh_5
        res["vdraws_lower"] = self.vdraws_lower
        res["vdrawsh_lower"] = self.vdrawsh_lower
        res["vdraws_upper"] = self.vdraws_upper
        res["vdrawsh_upper"] = self.vdrawsh_upper
        res["q_lower"] = self.q_lower
        res["q_upper"] = self.q_upper
        res["modeltype"] = self.modeltype
        return res
    ...
    def _read_in_sobol(self, q_lower, q_upper):
        ...
        for i in range(1, self.tc):
            read = open(sobol_draws_files[i], "r")
            lines = read.readlines()
            if lines[0] != "\n" and lines[1] != "\n":  # If it's nonempty
                self.so_draws = np.vstack(
                    (self.so_draws, np.loadtxt(sobol_draws_files[i]))
                )
        ...
        draws = self.so_draws
        p = self.p  # ^ Shorthand to make the next lines shorter
        ...
        self.tvidraws = draws[
            :, (ncol - 1 - p) : (ncol - 1)
        ]  # Columns 4 and 5 for p = 2
        self.vdraws = draws[:, ncol - 1].reshape(
            self.ndpost, 1
        )  # Column 6 for p = 2 (aLways last column)
    ...
    def sobol(self, q_lower=0.025, q_upper=0.975):
        ...
        with self.configfile.open("w") as tfile:
            for param in sobol_params:
                if (
                    type(param) != str and type(param) != int
                ):  # Makes minx & maxx into writable quantities, not arrays
                    for item in param:
        ...
        res["vijdraws"] = self.vijdraws
        res["tvidraws"] = self.tvidraws
        res["vdraws"] = self.vdraws
        res["sidraws"] = self.sidraws
        res["sijdraws"] = self.sijdraws
        res["tsidraws"] = self.tsidraws
        res["msi"] = self.msi
        res["msi_sd"] = self.msi_sd
        res["si_5"] = self.si_5
        res["si_lower"] = self.si_lower
        res["si_upper"] = self.si_upper
        res["msij"] = self.msij
        res["sij_sd"] = self.sij_sd
        res["sij_5"] = self.sij_5
        res["sij_lower"] = self.sij_lower
        res["sij_upper"] = self.sij_upper
        res["mtsi"] = self.mtsi
        res["tsi_sd"] = self.tsi_sd
        res["tsi_5"] = self.tsi_5
        res["tsi_lower"] = self.tsi_lower
        res["tsi_upper"] = self.tsi_upper
        res["q_lower"] = self.q_lower
        res["q_upper"] = self.q_upper
        res["modeltype"] = self.modeltype
        res["so_draws"] = self.so_draws
        return res
    ...
    def obt_save(self, post=None, dirname=None, postname="post_PyData", est=False):
        if est == False:
            ...
            if dirname == None:
                dirname = (
                    post  # b/c the user may have entered things in the wrong order
                )
            if dirname == None:
                sys.exit(
                    "Please specify directory name to save the estimator object.\n"
                )
